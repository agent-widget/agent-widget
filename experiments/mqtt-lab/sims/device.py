...
Simulates the future device behavior over MQTT:
  ...
  - QoS 1 subscriptions: agents/+/status, ota/announce, ota/{deviceId},
    ota/group/canary-{bucket}, ota/group/stable, ota/group/beta
  - Last-Will offline telemetry (retained) so the fleet server sees drops
  - automatic reconnect with backoff (mirrors the ESP32 MQTT task)
  ...
  - Wi-Fi drop / late-join failure injection for testing
...
from common import (  # noqa: E402
    DEVICE_PASS, KEEPALIVE, c, canary_group, events_topic, log, ota_device_topic,
    ota_group_topic, ota_result_topic, progress_bar, status_topic, telemetry_topic,
    build_event, build_ota_result, build_telemetry, OTA_ANNOUNCE, version_cmp,
)
...
class SimDevice:
    def __init__(self, args):
        ...
        self.client.username_pw_set(self.device_id, DEVICE_PASS)
    ...
    def _on_connect(self, client, userdata, flags, reason_code, properties):
        if reason_code.is_failure:
            if reason_code.value == 5:
                log("DEVICE", c(f"auth failed for {self.device_id} — run "
                                "scripts/add-device-user.sh "
    ...
    def _on_agent_status(self, topic, payload):
        ...
        if self._validate("agent_status", payload, topic):
            self.agents[agent_id] = payload
            st = payload.get("state", "?")
            pct = payload.get("progress")
            line = (f"{c(agent_id, STATE_COLOR_NAME(st))} {c(st, STATE_COLOR_NAME(st))}"
            ...
            log("DEVICE", f"▸ {line}")
            self._publish_event("status_rendered",
                                {"agentId": agent_id, "state": st})
    ...
    def _on_ota(self, topic, payload):
        ...
        threading.Thread(target=self._process_ota, args=(topic, payload),
                         daemon=True).start()
    ...
    def _head_check(self, url):
        try:
            ...
            with urllib.request.urlopen(req, timeout=5) as r:
                log("DEVICE", f"HTTPS download check: {r.status} {len(r.headers.get('Content-Length', ''))} bytes")
    ...
    def _panel_loop(self):
        ...
        while not self.stopping:
            ...
            for agent_id, st in sorted(self.agents.items()):
                color = STATE_COLOR_NAME(st.get("state"))
                pct = st.get("progress")
    ...
    def _wifi_drop_loop(self):
        """Periodically kill the link like a flaky Wi-Fi radio."""
        n = 0
        while not self.stopping:
            n += 1
            ...
            if self.stopping or not self._was_connected:
                continue
            log("DEVICE", c(f"wifi drop #{n}: link lost for "
                            f"{self.args.drop_duration}s (LWT fires)", "yellow"))
            try:
                self.client.disconnect()   # triggers on_disconnect -> LWT offline
            except Exception:
                pass
            time.sleep(self.args.drop_duration)
            # _on_disconnect already scheduled a reconnect
...
def main():
    ...
    ap.add_argument("--ca", default="../broker/certs/ca.crt", help="CA cert for TLS")
    ...
    ap.add_argument("--drop-every", type=float, default=0,
                    help="simulate a Wi-Fi drop every N seconds (0=off)")
