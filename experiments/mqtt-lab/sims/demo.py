...
Proves the future production behaviors on a local broker:
  ...
  7. recall guard: an older announce is rejected (anti-downgrade) and a
     min_version wall rejects upgrades (anti-rollback floor)
...
from common import (  # noqa: E402
    ...
    canary_groups_for_percent, c, status_topic, telemetry_topic,
    ota_result_topic, build_agent_status,
)
...
class Demo:
    ...
    def clear_retained(self):
        """Wipe retained state from previous runs (empty retained = clear).

        Without this, leftover retained telemetry/announces from an earlier
        demo run pollute the assertions (e.g. a device that never connected
        this run still reports the old version).
        """
        cleared = []
        ...
        # wait for the broker to process the clears
        ...
        print(f"    cleared {len(cleared)} retained topics from previous runs")
    ...
    def run(self):
        ...
        ok = self.collector.wait_until("collector sees retained state", lambda: True, 1)
        self.check("broker reachable on %s:%s" % (self.host, self.port), ok)
        if self.args.tls:
            self.check("TLS client connected", True)
        self.clear_retained()

        self.section("1. fleet boots (telemetry + LWT)")
        for dev, role in ROLE.items():
            extra = []
            if dev == "esp32s3-778899":
                extra = ["--fail-self-test"]
            if dev == "esp32s3-445566":
                extra = ["--offline-start", "45"]
            if dev == "esp32s3-112233":
                extra = []  # stopped later for the offline-queue test
            self.spawn_device(dev, *extra)
            print(f"    spawned {dev} ({role})")

        online_ok = self.wait_fleet_online([d for d in ROLE if d != "esp32s3-445566"],
                                           25)
        ...
        canary_groups = canary_groups_for_percent(40, FLEET)
        ...
        from common import build_ota_announce, default_ota_url, ota_group_topic
        for g in canary_groups:
            self.publish(self._announce("3.1.0", "2.0.0", "ota-demo-canary"),
                         ota_group_topic(g), qos=1, retain=False)
        canary_ok = self.collector.wait_until(
            "canary devices install 3.1.0",
            lambda: all(self.collector.version_of(d) == "3.1.0"
                        for d in canary_devs if d != "esp32s3-445566"
                        and ROLE[d] != "rollback-drill (fail self-test)"),
            self._t(12))
        self.check("canary devices upgraded to 3.1.0", canary_ok)
        # non-canary devices must have NO accepted event for this announce
        non_canary = [d for d in FLEET if d not in canary_devs]
        no_leak = all(not self.collector.events_of(d, "ota_accepted") for d in non_canary)
        self.check("non-canary devices untouched (no leak)", no_leak)

        self.section("4. OTA group rollout (stable cohort)")
        self.publish(self._announce("3.1.0", "2.0.0", "ota-demo-stable"),
                     "ota/group/stable", qos=1, retain=False)
        # The late joiner (esp32s3-445566) is offline by design and only ever
        # receives the retained broadcast (step 6), never non-retained groups.
        stable_targets = [d for d in FLEET if d not in canary_devs
                          and d != "esp32s3-445566"]
        stable_ok = self.collector.wait_until(
            "stable cohort upgrades to 3.1.0",
            ...
            self._t(12))
        self.check("stable cohort upgraded to 3.1.0", stable_ok)
        rb = "esp32s3-778899"
        rollback_ok = self.collector.wait_until(
            "rollback-drill device rolled back",
            lambda: any(e[1] == "ota_rolled_back" for e in self.collector.events_of(rb)),
            self._t(12))
        self.check("fail-self-test device rolled back, keeps fw "
                   f"{self.collector.version_of(rb)}", rollback_ok)

        self.section("5. offline device + persistent session (queued announce)")
        off = "esp32s3-112233"
        p, _ = self.procs[off]
        print(f"    SIGSTOP {off} (freeze the process; broker will see LWT offline)")
        ...
        lwt_ok = self.collector.wait_until(
            f"{off} reported offline via broker LWT",
            lambda: (lambda t: t and not t.get("online") and t.get("cause") == "lwt")(
                self.collector.telemetry.get(off)),
            40)
        self.check("broker detected the drop and fired LWT offline", lwt_ok)

        self.publish(self._announce("3.2.0", "2.0.0", "ota-demo-targeted"),
                     f"ota/{off}", qos=1, retain=False)
        print(f"    targeted 3.2.0 published while {off} is offline (queued)")
        time.sleep(1)
        p.send_signal(signal.SIGCONT)
        print(f"    SIGCONT {off} -> reconnects, persistent session delivers the queue")
        queued_ok = self.collector.wait_until(
            f"{off} installs the queued 3.2.0",
            lambda: self.collector.version_of(off) == "3.2.0", 30)
        self.check("queued announce delivered on reconnect, installed 3.2.0", queued_ok)
        others_untouched = all(self.collector.version_of(d) != "3.2.0"
                               for d in FLEET if d != off)
        self.check("no other device saw the targeted announce (ACL isolation)",
                   others_untouched)

        self.section("6. retained broadcast + late joiner")
        self.publish(self._announce("3.3.0", "2.0.0", "ota-demo-broadcast"),
                     "ota/announce", qos=1, retain=True)
        bc_ok = self.collector.wait_until(
            "online fleet installs 3.3.0 (except rollback-drill device)",
            lambda: all(self.collector.version_of(d) == "3.3.0"
                        for d in FLEET if d != "esp32s3-778899"
                        and self.collector.version_of(d) not in (None,)),
            self._t(14))
        self.check("broadcast upgrade to 3.3.0 (online devices)", bc_ok)

        late = "esp32s3-445566"
        late_ok = self.collector.wait_until(
            f"late joiner {late} connects and installs retained 3.3.0",
            lambda: self.collector.version_of(late) == "3.3.0", 45)
        self.check("late joiner received retained broadcast and installed", late_ok)
        rendered_ok = self.collector.wait_until(
            f"late joiner renders the retained AgentStatuses",
            lambda: len(self.collector.events_of(late, "status_rendered")) >= 3, 15)
        rendered = len(self.collector.events_of(late, "status_rendered"))
        self.check("late joiner rendered retained AgentStatuses (>=3)",
                   rendered_ok, f"({rendered} rendered)")
        leaked_events = [e for e in self.collector.events_of(late)
                         if e[1] == "ota_accepted" and
                         e[2].get("announceId") != "ota-demo-broadcast"]
        ...
        self.publish(self._announce("3.0.0", "3.3.0", "ota-demo-recall"),
                     "ota/announce", qos=1, retain=True)
        recall_ok = self.collector.wait_until(
            "devices reject the older recall announce",
            lambda: any(e[1] == "ota_rejected" and e[2] and
                        e[2].get("to") == "3.0.0" for e in self.collector.events), 12)
        self.check("older announce rejected (anti-downgrade recall guard)", recall_ok)

        self.publish(self._announce("3.4.0", "9.9.9", "ota-demo-minwall"),
                     "ota/announce", qos=1, retain=True)
        wall_ok = self.collector.wait_until(
            "devices reject above min_version wall",
            lambda: any(e[1] == "ota_rejected" and e[2] and
                        e[2].get("to") == "3.4.0" and
                        e[2].get("reason", "").startswith("current")
                        for e in self.collector.events), 12)
        self.check("upgrade above min_version wall rejected", wall_ok)
    ...
    def _announce(self, version, min_version, ann_id):
        from common import build_ota_announce, default_ota_url
        import hashlib
...
def main():
    ...
    args = ap.parse_args()
    if not broker_up(args.host, args.port):
